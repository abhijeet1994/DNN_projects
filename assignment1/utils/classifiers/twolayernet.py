...
class TwoLayerNet(object):
    ...
    def loss(self, X, y):
        ...
        w1, b1 = self.layer1.params
        w2, b2 = self.layer2.params
        layer2_input = self.layer1.feedforward(X)
        out = self.layer2.feedforward(layer2_input)
        #self.layer2.X = out
        #output = softmax(out)
        loss, dx2 = softmax_loss(out, y)
        ...
        dx1 = self.layer2.backward(dx2)
        dx = self.layer1.backward(dx1)
    ...
    def step(self, learning_rate=1e-5, optim='SGD', momentum=0.5):
        ...
        #            START OF YOUR CODE          #
        ################################################### 
        self.velocities = [momentum*self.velocities[i] + learning_rate*grads[i] for i in range(0, len(grads))]
        params = [params[i] - self.velocities[i] for i in range(0,len(params))]
