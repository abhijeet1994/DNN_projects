    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            for i in range(0,len(index)//batch_size):
                x_batch  =np.asarray([X[x] for x in index[i*batch_size:i*batch_size+200]])
                y_batch = np.asarray([y[x] for x in index[i*batch_size:i*batch_size+200]])
