...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    encode_outputs, encode_state = tf.nn.dynamic_rnn(encoding_model, embed  , dtype = tf.float32)
    return(encode_outputs, encode_state)
...
def my_optimizer(loss,grad_clip, learning_rate):
    ...
    new_optimizer = tf.train.AdamOptimizer(learning_rate)
    gradients = new_optimizer.compute_gradients(loss)
