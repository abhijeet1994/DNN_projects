...
class SGDmomentumOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        #velocitys = [momentum*velocitys[i] - (learning_rate * grads[i]) for i in range(0,len(velocitys))]
        #params = [params[i] - velocitys[i] for i in range(0,len(params))]
        
        #for k in range (0,len(params)):
        for k in grads:
            velocitys[k] = momentum*velocitys[k]
            velocitys[k]-= learning_rate * grads[k]
...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            cache[k] = gamma*cache[k]
            cache[k] += ((1-gamma)*grads[k]*grads[k])
            params[k] -= (learning_rate/((cache[k]+eps)**0.5))*grads[k]
        self.cache = cache
        model.params = params
        ###################################################
        #raise NotImplementedError
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            momentums[k] = beta1*momentums[k]
            momentums[k] += (1-beta1)*grads[k]
            mom = momentums[k]/(1-(beta1**t))
            velocitys[k] = beta2*velocitys[k]
            velocitys[k] += ((1-beta2)*grads[k]*grads[k])
            vels = velocitys[k]/(1-(beta2**t))
            params[k] -= ((learning_rate*mom)/((vels**0.5)+eps))
        self.momentums = momentums
        self.velocitys = velocitys
        self.t = t
        model.params = params
