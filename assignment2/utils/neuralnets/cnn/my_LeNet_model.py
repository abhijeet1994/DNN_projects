...
def my_LeNet(input_x, input_y, is_training,
    ...
    fc_layer_1 = fc_layer(input_x=fc_layer_0.output(),
                          in_size=fc_units[0],
                          out_size=fc_units[1],output_size,
                          rand_seed=seed,
                          activation_function=tf.nn.relu,
                          index=1)
...
def my_training_task4(X_train, y_train, X_val, y_val, 
    ...
    trial =X_train.reshape(-1,1,28,28).transpose(0,2,3,1)
    X_val=X_val.reshape(-1,1,28,28).transpose(0,2,3,1)
    img_len = trial.shape[1]
