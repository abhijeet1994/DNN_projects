...
def my_LeNet(input_x, input_y, is_training,
    ...
    norm_layer_0 = norm_layer(input_x = conv_layer_0.output(), is_training = is_training)
    ...
    norm_layer_1 = norm_layer(input_x = conv_layer_1.output(), is_training = is_training)
    ...
    norm_layer_2 = norm_layer(input_x = conv_layer_2.output(), is_training = is_training)
    ...
    norm_layer_3 = norm_layer(input_x = conv_layer_3.output(), is_training = is_training)
    ...
    # flatten
    pool = pooling_layer_3
    pool_shape = pool.output().get_shape()
    img_vector_length = pool_shape[1].value * pool_shape[2].value * pool_shape[3].value
    flatten = tf.reshape(pool.output(), shape=[-1, img_vector_length])
    ...
    norm_fc_layer_0 = norm_layer(input_x = fc_layer_0.output(), is_training = is_training)

    
    fc_layer_1 = fc_layer(input_x=norm_fc_layer_0.output(),
                          in_size=fc_units[0],
                          out_size=fc_units[1],
                          rand_seed=seed,
                          activation_function=tf.nn.relu,
    ...
    norm_fc_layer_1 = norm_layer(input_x = fc_layer_1.output(), is_training = is_training)
    
    fc_layer_2 = fc_layer(input_x=norm_fc_layer_1.output(),
                          in_size=fc_units[1],
                          out_size=output_size,
                          rand_seed=seed,
                          activation_function=None,
                          index=2)
    norm_fc_layer_2 = norm_layer(input_x = fc_layer_2.output(), is_training = is_training)
    # output
    out = norm_fc_layer_2.output()
    # saving the parameters for l2_norm loss
    conv_w = [conv_layer_0.weight,conv_layer_1.weight,conv_layer_2.weight,conv_layer_2.weight]#,conv_layer_3.weight]
...
def my_training_task(X_train, y_train, X_val, y_val, 
    ...
    trial=X_train.reshape(-1,3,X_train.shape[1],X_train.shape[2]).transpose(0,2,3,1)
    X_val=X_val.reshape(-1,3,X_train.shape[1],X_train.shape[2]).transpose(0,2,3,1)
    print("Xtrain shape={}".format(X_train.shape))
    img_len = X_val.shape[1]
    channel_num = trial.shape[-1]
    print("Channel_size={}".format(channel_num))
    #gen = ImageGenerator(X_train.reshape(-1,3,128,128).transpose(0,2,3,1), y_train)
    ...
    with tf.Session() as sess:
        ...
        for epc in range(epoch):
            ...
            #X_train , y_train = gen.next_batch_gen(y_train.shape[0], shuffle=True)
            #print(X_train.shape)
...
def testing(X_input, pre_model, batch_size):
    N = X_input.shape[0]
    with tf.name_scope('inputs'):
        x = tf.placeholder(shape=[None, img_len, img_len, channel_num], dtype=tf.float32)
        y = tf.placeholder(shape=[None, ], dtype=tf.int64)
        is_training = tf.placeholder(tf.bool, name='is_training')
    output, __ = my_LeNet(xs, ys, is_training,
                         img_len=img_len,
                         channel_num=channel_num,
                         output_size=5,
                         conv_featmap=conv_featmap,
