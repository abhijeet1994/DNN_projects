...
class MLP(object):
    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                x, data = bn_forward(x, gamma, beta, bn_params[i], "train")
                cache_name = "bn_data_{}".format(i)
                cache[cache_name] = data
                
                """
                w_mean = np.mean(w, axis = 0)
                w_std = np.var(w, axis = 0)
                w = (w-w_mean)/w_std
                b_mean = np.mean(b, axis = 0)
                b_std = np.var(b, axis = 0)
                b = (b-b_mean)/b_std
                """
            ###############################################
            ...
            x,mask = dropout_forward(x, dropout_config, "train")
            cache[cache_name] = mask
            ###############################################
            # END OF DROPOUT                              #
            ###############################################
        # The last layer
        ...
        for i in range(num_layers - 1):
            ...
            #raise NotImplementedError
            if use_bn:
                dx, dgamma, dbeta = bn_backward(dx, cache["bn_data_{}".format(j)])
    ...
    def predict(self, X):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                gamma, beta = params["bn_gamma_{}".format(i)], params["bn_beta_{}".format(i)]
                x, data = bn_forward(x, gamma, beta, bn_params[i], "test")
            ...
            #raise NotImplementedError
            if dropout_config['enabled']:
                x, mask_not_to_be_used = dropout_forward(x, dropout_config, "test")
            ###############################################
            # END OF DROPOUT                              #
            ###############################################
        # The last layer
