...
def conv2d_forward(x, w, b, pad, stride):
    ...
    out = np.zeros((x.shape[0],new_height,new_width,w.shape[3]))
    for i in range(out.shape[0]):
        for j in range(out.shape[3]):
            for k in range(new_height):
                for l in range(new_width):
                    out[i,k,l,j] = np.sum(np.multiply(x_pad[i,k*stride:(k*stride)+w.shape[0],l*stride:(l*stride)+w.shape[1],:],w[:,:,:,j])) + b[j]
    return out
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    print(batch, height, width, channels,filter_height, filter_width, num_of_filters, height_new, width_new)
    x_pad = np.zeros((x.shape[0],x.shape[1]+(2*pad),x.shape[2]+(2*pad),x.shape[3]))
    x_pad[:,pad:(-pad),pad:(-pad),:] = x
    dx = np.zeros_like(x)
        ...
        for j in range(new_height):
            for k in range(new_width):
                for l in range(channels):
                    out[i,j,k,l]= x[i,j*stride:(j*stride)+pool_size,k*stride:(k*stride)+pool_size,l].max()
    return out
...
def max_pool_backward(dout, x, pool_size, stride):
    ...
    for i in range(new_height):
        for j in range(new_width):
            small_matrix = x[:,i*stride:(i*stride)+pool_size,j*stride:(j*stride)+pool_size,:]
            maximum = np.max(small_matrix,axis=(1,2))[:,None,None,:]
            correct_elements = (small_matrix==maximum)
            new_small_dout = correct_elements * ((dout[:,i,j,:])[:,None,None,:])
            dx[:,i*stride:(i*stride)+pool_size,j*stride:(j*stride)+pool_size,:] += new_small_dout 
    return dx